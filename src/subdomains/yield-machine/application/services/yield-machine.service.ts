...
export class YieldMachineService {
  ...
  async create({ command, parameters }: TransactionInput): Promise<string> {
    ...
    if (command === TransactionCommand.ACCOUNT_TO_ACCOUNT) {
      const isAllowed = await this.isSendAllowed(parameters);
      if (!isAllowed) throw new ForbiddenException('Send parameters are not allowed');
    }
  ...
  private async isSendAllowed(parameters: SendTokenParameters): Promise<boolean> {
    ...
    return allowedAddresses.includes(parameters.from) && allowedAddresses.includes(parameters.to);
  }
