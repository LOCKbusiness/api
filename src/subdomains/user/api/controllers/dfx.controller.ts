...
export class DfxController {
  ...
  async handoverDfxWebhook(@RealIP() ip: string, @Body() dto: WebhookDto) {
    ...
    switch (dto.type) {
      case WebhookType.KYC_CHANGED:
        await this.onKycChanged(dto, user);
        break;

      case WebhookType.KYC_FAILED:
        await this.onKycFailed(dto, user);
        break;
    }
  }
  ...
  private async onKycChanged(dto: WebhookDto, user: User): Promise<void> {
  ...
  private async onKycFailed(dto: WebhookDto, user: User): Promise<void> {
    if (user.mail) {
      await this.notificationService.sendMail({
        type: MailType.USER,
        input: {
